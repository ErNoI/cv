  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <div className="flex gap-1 shrink-0">
        {[1, 2, 3].map((i) => (
          <span key={i} className={`text-base leading-none ${i <= tier ? "text-action" : "text-accent"}`}>
            ●
          </span>
        ))}
      </div>
...
const SkillCategory: FC<{ name: string; skills: skillProps[]; cols?: number }> = ({
  name,
  skills,
  cols = 2,
}) => {
  ...
  return (
    <div className="rounded-md bg-secondary px-6 py-5 shadow-lg">
      <p className="mb-4 text-sm font-bold uppercase tracking-widest text-action">{name}</p>

      {/* Mobile: single column */}
      <div className="flex flex-col gap-3 sm:hidden">
        {sorted.map((s) => (
          <SkillRow key={s.skillName} skillName={s.skillName} percentage={s.percentage} />
        ))}
      </div>
      ...
      <div className="hidden sm:flex gap-0">
        {columns.map((col, i) => (
          <div key={i} className="flex flex-1">
            {i > 0 && <div className="mx-5 border-l border-action/40 self-stretch" />}
  ...
  return (
    <div id="skills" className="pb-16 pt-7 shadow-inner">
      <div className="flex flex-col items-center">
        ...
        <div className="mx-auto w-11/12 sm:w-3/4 flex flex-col gap-4">
          <SkillCategory name="Backend" skills={grouped[0].skills} cols={3} />
          <div className="grid gap-4 sm:grid-cols-2">
            {grouped.slice(1).map((g) => (
              <SkillCategory key={g.name} name={g.name} skills={g.skills} cols={2} />
            ))}
          </div>
        </div>
      </div>
